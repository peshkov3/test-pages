...
function InformerCreator(id, _percentageFrom, _percentageTo) {
    ...
    /**
     * Find appropriate plase in article, and paste Informer Div to it
     *
     * @param insertedElementId - ID of the element that will be moved to appropriate plase
     * @private
     */
    function _create(insertedElementId) {
        ...
        var offsetHeightFrom = articleHeight * percentageFrom / 100;
        var offsetHeightTo = articleHeight * percentageTo / 100;
        var accumulativeSum = 0;

        // search for the appropriate position
        for (var i = 0; i < articleInDOM.children.length; i++) {

            var element = articleInDOM.children[i];

            accumulativeSum += element.clientHeight;

            if (accumulativeSum >= offsetHeightFrom &&
                accumulativeSum <= offsetHeightTo &&
                _canInsertToBlock(element)
            ) {
                try {
                    // find MarketGidComposite
